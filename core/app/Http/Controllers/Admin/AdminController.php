...
class AdminController extends Controller
{
    ...
    function filterUsersBasedOnAccess($query) {
        if (!can_access('access-all-users')) {
            if (can_access('manage-sales-leads')) {
                $query->whereDoesntHave('wallets', function ($query) {
                    $query->where('balance', '!=', 0);
                });
            } elseif (can_access('manage-retention-leads')) {
                $query->whereHas('wallets', function ($query) {
                    $query->where('balance', '!=', 0);
                });
            } else {
                $query->where('owner_id', auth()->guard('admin')->user()->id);
            }
        }
        return $query;
    }
