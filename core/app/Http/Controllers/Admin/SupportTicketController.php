...
class SupportTicketController extends Controller
{
    ...
    public function tickets(Request $request)
    {
        ...
        $items = SupportTicket::orderBy('id','desc')->with('user')->paginate($perPage);
    ...
    public function closedTicket(Request $request)
    {
        ...
        $perPage = $request->get('per_page', 25);
        $items = SupportTicket::where('status',Status::TICKET_CLOSE)->orderBy('id','desc')->with('user')->paginate($perPage);
