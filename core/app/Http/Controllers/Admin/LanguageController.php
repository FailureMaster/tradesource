...
class LanguageController extends Controller
{
    ...
    public function langManage(Request $request, $lang = false)
    {
        $pageTitle  = 'Language Manager';
        $perPage    = $request->get('per_page', 25);
        $languages  = Language::with('languageCountries')->orderBy('is_default', 'desc')->paginate($perPage);
        return view('admin.language.lang', compact('pageTitle', 'languages', 'perPage'));
    }
    ...
    public function langEdit(Request $request, $id)
    {
        ...
        return view('admin.language.edit_lang', compact('pageTitle', 'json', 'lang', 'list_lang', 'perPage'));
    }
   ...
   public function addCountriesToLanguage(Request $request)
    {
        DB::transaction(function () use ($request) {
            LanguageCountry::create([
    ...
    public function languageSettingView(Request $request, Language $language)
    {
        $pageTitle = 'Language Settings';
        $perPage    = $request->get('per_page', 25);
        
        $language->load('languageCountries');
        $languages  = Language::with('languageCountries')->orderBy('is_default', 'desc')->paginate($perPage);

        return view('admin.language.setting', compact('pageTitle', 'language','languages', 'perPage'));
    }
    
  public function deleteCountryFromLanguage($id)
    {
        DB::transaction(function () use ($id) {
            $languageCountry = LanguageCountry::findOrFail($id);

            $languageCountry->delete();
        });
    
        return redirect()->back()->with('success', 'Country removed from language successfully.');
    }
}
