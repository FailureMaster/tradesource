...
class GeneralSettingController extends Controller
{
    ...
    public function update(Request $request)
    {
        $request->validate([
            ...
            'auth_color'                   => ['nullable', 'regex:/^[a-f0-9]{6}$/i'],
            'timezone'                     => 'required',
        ...
        $general->auth_color                 = str_replace('#', '', $request->auth_color);
        $general->save();
    ...
    public function systemConfigurationSubmit(Request $request)
    {
        ...
        $general->auth_redirect  =  $request->auth_redirect ? Status::ENABLE : Status::DISABLE;
        $general->allows_demo_account  =  $request->allows_demo_account ? Status::ENABLE : Status::DISABLE;
        $general->dashboard       =  $request->dashboard ? Status::ENABLE : Status::DISABLE;
        $general->withdraw_money  =  $request->withdraw_money ? Status::ENABLE : Status::DISABLE;
        $general->ip_address_check  =  $request->ip_address_check ? Status::ENABLE : Status::DISABLE;
        $general->save();
